    ...
    mFiles(aRemoveElement, this)
{
    ...
    connect(&mFiles, SIGNAL(clickLink(QByteArray)), this, SIGNAL(clickLink(QByteArray)));
}
...
void ViewPostWidget::save()
{
    ...
    execQuery(QString("UPDATE messages SET message = '%1' WHERE id = %2")
              .arg(ui->mpEdit->toPlainText())
              .arg(vId));
