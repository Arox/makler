...
void TableModelApartmentClient::clear()
{
    FindTableModel::clear("apartment");
}

void TableModelApartmentClient::addToOrder()
{
    clear();
    ...
    vSql //<< select()
         ...
         //<< "AND types.type = 'apartment'";
    execQuery(QString("INSERT INTO objectstoorders (object_fk, object_in_order_fk) (%1)")
              .arg(vSql.join(" ")));
}

QVariant TableModelApartmentClient::data(const QModelIndex &index, int role) const
{
...
void TableModelApartmentClient::load()
{
    QStringList vSql;
    vSql << select()
